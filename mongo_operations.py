...
def insert(transcript, sentiment_polarity, emoji, timestamp):
    _id=uuid.uuid4()
    record = {
        '_id': str(_id),
...
def augment_and_insert(transcript, timestamp):
    sentiment_polarity = calculate_sentiment_polarity(transcript)
    emoji = emoji_mapper(sentiment_polarity)
    insert(transcript, sentiment_polarity, emoji, timestamp)